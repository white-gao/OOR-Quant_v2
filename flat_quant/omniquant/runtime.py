...
paper-style asymmetric integer LWC with independently learned upper/lower
clipping factors and a zero point.  Both variants operate per output channel
by default and optionally per consecutive input-feature group.
...
SymmetricLWCMode = Literal["absmax", "two_sided"]
...
class OmniQuantConfig:
    ...
    # FP/symmetric QDQ normally learns one absmax shrink factor. The
    # two-sided ablation retains the same zero-centered codebook and scale but
    # learns independent positive/negative clipping factors before QDQ.
    symmetric_lwc_mode: SymmetricLWCMode = "absmax"
    # Zero preserves one quantization range per output-channel row. A positive
    # value splits every row into consecutive groups along in_features.
    ...
    def validate(self) -> None:
        ...
        if self.symmetric_lwc_mode not in ("absmax", "two_sided"):
            raise ValueError(
                "symmetric_lwc_mode must be 'absmax' or 'two_sided'."
            )
        ...
        if (
            ...
            raise ValueError(
                "LFQ requires a positive lfq_loss_weight or "
...
class _TrainableSymmetricLinear(nn.Module):
    """Frozen Linear with trainable channel/group LWC and optional LET scales.
    ...
    symmetric INT/FP LWC or paper-style asymmetric integer LWC. A positive
    ``config.weight_group_size`` gives every consecutive input group its own
    learned range and quantization scale.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.weight_group_size = normalize_weight_group_size(
            config.weight_group_size
        )
        self.num_weight_groups = (
            1
            if self.weight_group_size is None
            else math.ceil(linear.in_features / self.weight_group_size)
        )
        parameter_shape = (
            (linear.out_features, 1)
            if self.weight_group_size is None
            else (linear.out_features, self.num_weight_groups, 1)
        )
        symmetric_lwc_mode = getattr(config, "symmetric_lwc_mode", "absmax")
        if (
            config.weight_quant_scheme == "symmetric"
            and symmetric_lwc_mode == "absmax"
        ):
    ...
    def lwc_parameters(self) -> list[nn.Parameter]:
        if (
            self.config.weight_quant_scheme == "symmetric"
            and getattr(self.config, "symmetric_lwc_mode", "absmax") == "absmax"
        ):
    ...
    def _group_weight(
        ...
        """Return row groups, a tail-validity mask, and the padded width."""
        if self.weight_group_size is None:
            return weight, None, self.in_features
        padded_in_features = self.num_weight_groups * self.weight_group_size
        padding = padded_in_features - self.in_features
        ...
        if padding:
            valid_mask = (
                ...
                .lt(self.in_features)
                .reshape(1, self.num_weight_groups, self.weight_group_size)
            )
        return grouped_weight, valid_mask, padded_in_features

    def _restore_grouped_weight(
        self,
        ...
        *,
        ...
        restored = grouped_weight.reshape(self.out_features, padded_in_features)[
            :, : self.in_features
        ]
        return (
            restored
    ...
    def _qdq_weight_tensor(self, weight: torch.Tensor, *, use_ste: bool) -> torch.Tensor:
        ...
        if self.config.weight_quant_scheme == "asymmetric":
            ...
            if valid_mask is None:
                upper = grouped_weight.amax(dim=reduce_dim, keepdim=True)
                lower = grouped_weight.amin(dim=reduce_dim, keepdim=True)
            else:
                upper = grouped_weight.masked_fill(
                    ~valid_mask, -torch.inf
                ).amax(dim=reduce_dim, keepdim=True)
                lower = grouped_weight.masked_fill(
                    ~valid_mask, torch.inf
                ).amin(dim=reduce_dim, keepdim=True)
            upper = upper * upper_ratio
            lower = lower * lower_ratio
            ...
            q = (round_fn(grouped_weight / scale) + zero_point).clamp(
                min=0.0, max=qmax
            )
        ...
        symmetric_lwc_mode = getattr(
            self.config,
            "symmetric_lwc_mode",
            "absmax",
        )
        quantization_input = grouped_weight
        if symmetric_lwc_mode == "two_sided":
            ...
            quantization_input = torch.minimum(
                torch.maximum(grouped_weight, lower),
                upper,
            )
            threshold = quantization_input.abs().amax(
                dim=reduce_dim,
                keepdim=True,
            )
        else:
            ...
            # Keep the transformed-weight absmax in the graph. Detaching it
            # makes reciprocal LET factors cancel to an exact zero gradient.
            threshold = grouped_weight.abs().amax(
                dim=reduce_dim,
                keepdim=True,
            ) * ratio
        if self.config.weight_quant_format == "fp8_e4m3fn":
            scale = threshold.clamp_min(self.config.eps) / FP8_MAX
            normalized = (quantization_input / scale).clamp(
            ...
            q = round_fn(quantization_input / scale).clamp(
                min=-qmax, max=qmax
            )
            qdq = q * scale
        return self._restore_grouped_weight(
    ...
    for epoch in range(1, config.epochs + 1):
        ...
        for batch_idx, (fp_batch, quant_batch) in enumerate(zip(fp_inputs, quant_inputs)):
            (
                ...
                quant_batch,
                evaluate_lfq=(
                    config.lfq_loss_weight > 0.0
                    or config.lfq_boundary_loss_weight > 0.0
                ),
            )
            if not torch.isfinite(loss):
...
def _validate_omniquant_checkpoint(
    ...
    *,
    ...
    expected_config: dict[str, Any] = {
        ...
        "activation_quant_format": config.activation_quant_format,
        "weight_quant_scheme": config.weight_quant_scheme,
        "symmetric_lwc_mode": config.symmetric_lwc_mode,
        "weight_group_size": config.weight_group_size,
    ...
    *,
    ...
    for layer_idx in selected_layer_indices:
        ...
        print(
            ...
            f"weight_group_size={config.weight_group_size or 'per_channel'}"
        )
    return summaries
