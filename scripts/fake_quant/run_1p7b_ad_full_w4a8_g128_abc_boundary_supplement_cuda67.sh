...
# Supplement the completed W4A8-g128 core matrix with two final-layer arms:
...
#
# Usage:
#   bash scripts/fake_quant/run_1p7b_ad_full_w4a8_g128_abc_boundary_supplement_cuda67.sh
#
# Useful overrides:
#   DRY_RUN=1 bash scripts/fake_quant/run_1p7b_ad_full_w4a8_g128_abc_boundary_supplement_cuda67.sh
#   GPUS=4,5 bash scripts/fake_quant/run_1p7b_ad_full_w4a8_g128_abc_boundary_supplement_cuda67.sh
...
PREFIX_OUTPUT_DIR="${W4_PREFIX_OUTPUT_DIR:-${ARTIFACTS_ROOT}/results/fake_quant/recommender/1p7b_ad_fp4w_fp8a_lwc_g128_abc_boundary_prefix128_final512_heldout512/shared_mse_lwc_prefix_calib128}"
PREFIX_CHECKPOINT_DIR="${PREFIX_OUTPUT_DIR}/${MODEL_NAME}/ad/omniquant_calibration"

RTN_EVAL_DIR="${W4_ROOT}/rtn_ad_${EVAL_SAMPLE_SIZE}"
MSE_EVAL_DIR="${W4_ROOT}/omniquant_lwc_mse_ad_${EVAL_SAMPLE_SIZE}"
ABC_ONLY_CALIB_DIR="${W4_ROOT}/omniquant_lwc_abc_only_final${FINAL_CALIB_SAMPLES}_calibration"
ABC_ONLY_CHECKPOINT_DIR="${ABC_ONLY_CALIB_DIR}/${MODEL_NAME}/ad/omniquant_calibration"
ABC_ONLY_EVAL_DIR="${W4_ROOT}/omniquant_lwc_abc_only_ad_${EVAL_SAMPLE_SIZE}"
REFERENCE_BOUNDARY_EVAL_DIR="${W4_ROOT}/omniquant_lwc_abc_boundary_w${REFERENCE_BOUNDARY_LOSS_WEIGHT}_ad_${EVAL_SAMPLE_SIZE}"
EXTRA_BOUNDARY_CALIB_DIR="${W4_ROOT}/omniquant_lwc_abc_boundary_w${EXTRA_BOUNDARY_LOSS_WEIGHT}_final${FINAL_CALIB_SAMPLES}_calibration"
EXTRA_BOUNDARY_CHECKPOINT_DIR="${EXTRA_BOUNDARY_CALIB_DIR}/${MODEL_NAME}/ad/omniquant_calibration"
EXTRA_BOUNDARY_EVAL_DIR="${W4_ROOT}/omniquant_lwc_abc_boundary_w${EXTRA_BOUNDARY_LOSS_WEIGHT}_ad_${EVAL_SAMPLE_SIZE}"
SUMMARY_PATH="${SUMMARY_PATH:-${RESULTS_ROOT}/w4a8_g128_abc_boundary_sweep_summary.json}"
...
write_summary() {
    ...
    "${PYTHON_BIN}" - \
        "${SUMMARY_PATH}" \
        "${MODEL_NAME}" \
        w4a8_rtn_g128 "${RTN_EVAL_DIR}/eval_results.json" \
        w4a8_omniquant_lwc_mse_g128 "${MSE_EVAL_DIR}/eval_results.json" \
        w4a8_abc_only_g128 "${ABC_ONLY_EVAL_DIR}/eval_results.json" \
        w4a8_abc_boundary_w0.3_g128 "${REFERENCE_BOUNDARY_EVAL_DIR}/eval_results.json" \
        w4a8_abc_boundary_w0.5_g128 "${EXTRA_BOUNDARY_EVAL_DIR}/eval_results.json" <<'PY'
import json
import os
import sys
...
print("[W4 sweep summary] method                              " + " ".join(f"{name:>13}" for name in metric_names))
for label, _path in pairs:
    ...
    print(f"[W4 sweep summary] {label:<35} " + " ".join(cells))
print(f"[W4 sweep summary] output={summary_path}")
PY
}

echo "[protocol] model=${MODEL_PATH} task=ad seed=42"
echo "[protocol] supplement=W4A8-g128 ABC-only and ABC+0.5-boundary"
...
if [[ "${DRY_RUN}" != "1" ]] && ! checkpoint_range_complete "${PREFIX_CHECKPOINT_DIR}" 0 "${PREFIX_LAST_LAYER}"; then
    echo "The completed W4A8-g128 prefix is missing or incomplete: ${PREFIX_CHECKPOINT_DIR}" >&2
    echo "Run the core6 launcher first, or set W4_PREFIX_OUTPUT_DIR to a complete prefix." >&2
    exit 4
fi
...
if ! wait "${abc_only_pid}"; then
    echo "[W4A8-g128] ABC-only final-layer calibration failed." >&2
    calibration_failed=1
fi
if ! wait "${extra_boundary_pid}"; then
    echo "[W4A8-g128] ABC+0.5 final-layer calibration failed." >&2
    calibration_failed=1
fi
...
echo "[done] W4A8-g128 ABC-only and ABC+0.5-boundary AD-${EVAL_SAMPLE_SIZE} completed."
